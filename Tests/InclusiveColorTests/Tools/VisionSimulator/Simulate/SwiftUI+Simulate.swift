...
@available(OSX 11.0, iOS 14, *)
class SwiftUISimulateTests: XCTestCase {
    ...
    func test10kInputColors_FullySimulated_Requires90ms_onM1MacMini() throws {
        ...
        let test = ICTestCases.LargerN.make10kcolors().map { Color($0) }
        
        var results = [[ICColorVisionType : Color]]()
        measure {
    ...
    func testSimulateSingleColorFunctions_MatchFirefoxAndColorspaciousImplementations() throws {
        let sut = ICVisionSimulator_Machado()
        let cases = ICTestCases.Machado.Google.cases.map { $0.mapValues { Color($0) }}
        
        cases.forEach { test in
            let color: Color = test[.typicalTrichromacy]!
    ...
    func testSimulateColorArrayFunctions_MatchFirefoxAndColorspaciousImplementations() throws {
        ...
        let test = cases.flatMap { $0.compactMap { $0.key == .typicalTrichromacy ? Color($0.value) : nil } }
        let expProtan = cases.flatMap { $0.compactMap { $0.key == .protanopia ? Color($0.value) : nil } }
        let expDeutan = cases.flatMap { $0.compactMap { $0.key == .deuteranopia ? Color($0.value) : nil } }
        let expTritan = cases.flatMap { $0.compactMap { $0.key == .tritanopia ? Color($0.value) : nil } }
        let expMonochromat = cases.flatMap { $0.compactMap { $0.key == .monochromacy ? Color($0.value) : nil } }
